...
SELECT create_hypertable('event_logs', 'block_timestamp', chunk_time_interval => INTERVAL '1 day', if_not_exists => TRUE);
...
ALTER TABLE event_logs SET (timescaledb.compress, timescaledb.compress_segmentby = 'address', timescaledb.compress_orderby = 'block_timestamp DESC, log_index ASC');
...
SELECT add_retention_policy('event_logs', INTERVAL '1 year', if_not_exists => TRUE);
